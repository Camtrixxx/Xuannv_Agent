...
def _default_path(env_name: str, relative_path: str) -> str:
    return os.getenv(env_name, str(DEFAULT_AEF_CODE_ROOT / relative_path))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--host", default=os.getenv("AEF_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=_get_int_env("AEF_PORT", 7862))
    parser.add_argument(
        "--config",
        default=_default_path("AEF_CONFIG", "configs/yajiang_v1_2_continue_200.yaml"),
    )
    parser.add_argument(
        "--manifest",
        default=_default_path("AEF_MANIFEST", "data/full_npy/train.jsonl"),
    )
    parser.add_argument(
        "--deploy-model",
        default=_default_path(
            "AEF_DEPLOY_MODEL",
            "outputs/aef_hyh_yajiang_v1_2_continue_200/exports/aef_hyh_yajiang_v1_2_continue_200_deploy.pt",
        ),
    )
    parser.add_argument(
        "--cache-dir",
        default=_default_path("AEF_CACHE_DIR", "outputs/aef_inference_service_v1_2_continue_200"),
    )
    parser.add_argument("--device", default=os.getenv("AEF_DEVICE", "auto"))
