...
class AEFRunnerConfig:
    config_path: Path = field(
        default_factory=lambda: _default_path("AEF_CONFIG", "configs/yajiang_v1_2_continue_200.yaml")
    )
    manifest_path: Path = field(default_factory=lambda: _default_path("AEF_MANIFEST", "data/full_npy/train.jsonl"))
    deploy_model_path: Path = field(
        default_factory=lambda: _default_path(
            "AEF_DEPLOY_MODEL",
            "outputs/aef_hyh_yajiang_v1_2_continue_200/exports/aef_hyh_yajiang_v1_2_continue_200_deploy.pt",
        )
    )
    cache_dir: Path = field(
        default_factory=lambda: _default_path("AEF_CACHE_DIR", "outputs/aef_inference_service_v1_2_continue_200")
    )
    device: str = field(default_factory=lambda: os.getenv("AEF_DEVICE", "auto"))
    seed: int = 42
